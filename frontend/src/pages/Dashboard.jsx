  ...
  const quickActions = [
    {
      icon: "send",
      label: "Send Money",
      sub: "Transfer to any campus peer",
    ...
    },
    {
      ...
      label: "Pay via QR",
      sub: "Instant scan and pay at kiosks",
    ...
    },
    {
      icon: "receipt_long",
      label: "View History",
      sub: "Review all your transactions",
          ...
          >
            ...
            >
              <div>
                ...
                <h1 className="font-headline font-bold text-5xl text-on-surface">
                  Hello, {user?.name?.split(" ")[0]} 👋
                </h1>
              </div>
              <button
                onClick={() => navigate("/notifications")}
                className="relative p-4 rounded-2xl bg-surface-low hover:bg-surface-container transition-colors"
              >
                <span className="material-icons text-3xl text-on-surface-variant">
                  notifications
                </span>
            ...
            <motion.div
              variants={stagger.item}
              className="relative rounded-[2rem] bg-card-gradient p-12 text-white overflow-hidden shadow-float"
            >
              <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,rgba(255,255,255,0.08),transparent_60%)]" />
              <div className="relative z-10">
                <p className="text-white/70 text-sm uppercase tracking-widest font-body mb-4 font-semibold">
                  Total Balance
                </p>
                <p className="font-headline font-bold text-7xl mb-2 tracking-tight">
                  {fmt(wallet?.balance)}
                </p>
                ...
                <div className="mt-10 flex gap-12">
                  <div>
                    ...
                    <p className="font-headline font-bold text-2xl text-emerald-300">
                      +{fmt(wallet?.monthlyInflow)}
                    </p>
                  </div>
                  <div>
                    <p className="text-white/60 text-sm mb-2 font-medium">
                    ...
                    <p className="font-headline font-bold text-2xl text-pink-300">
                      -{fmt(wallet?.monthlyOutflow)}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <motion.div variants={stagger.item}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {quickActions.map((a) => (
                  ...
                  >
                    <div
                      className={`w-16 h-16 rounded-2xl ${a.color} flex items-center justify-center mb-6`}
                    >
            ...
            <motion.div variants={stagger.item}>
              ...
              <div className="card p-4 space-y-2">
                {txs.length === 0 ? (
                  <div className="p-12 text-center text-on-surface-variant font-body text-base">
                    No transactions yet
                  </div>
                ) : (
                  txs.map((tx, i) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="flex justify-between items-center p-3 rounded-xl hover:bg-surface-low transition-colors cursor-pointer group"
                      >
                        <div className="flex items-center gap-4">
                          <div
                            className={`w-12 h-12 rounded-full flex items-center justify-center ${isIncoming ? "bg-tertiary/10" : "bg-error/10"}`}
                          >
                            <span
                              className={`material-icons ${isIncoming ? "text-tertiary" : "text-error"}`}
                            >
                        ...
                        <p
                          className={`font-headline font-bold text-2xl ${isIncoming ? "text-tertiary" : "text-on-surface"}`}
                        >
                          {isIncoming ? "+" : "-"}
                          {fmt(tx.amount)}
                        </p>
            ...
            <motion.div
              variants={stagger.item}
              className="card p-8 flex items-center gap-6 shadow-sm border border-surface-container"
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-on-surface-variant text-base font-body">
                  You're $150 away from your "New Laptop" goal. Keep it up!
                </p>
              </div>
